...
def build_surface_plot(surface, details, file):
	...
	X = np.arange(0, 1, 1.0 / surface.shape[0])
	Y = np.arange(0, 1, 1.0 / surface.shape[1])

	X2D, Y2D = np.meshgrid(X, Y)
	fig = plt.figure()
	ax = Axes3D(fig)
	ax.plot_surface(X2D, Y2D, surface)
	plt.suptitle("threads: {}, iterations: {}, runtime: {}s, dim: {}, eps: {}".format(
...
def read_results(directory):
	...
	details = pd.DataFrame(details, columns=(
		"NumThreads", "Iterations", "Runtime", "Dim", "Eps"))
	details = details.apply(pd.to_numeric, errors="ignore").groupby(
		['NumThreads', 'Dim'], as_index=False).mean()
	return details


# read_results("lab2_results")
details = pd.read_csv("dirichlet_benchmark.csv").groupby(
	['NumThreads', 'Dim'], as_index=False).mean()
build_efficiency_plots(details, "d_plots")

df = pd.read_csv("surface_8_100.csv", header=None, sep=" ")
build_surface_plot(np.array(df[:-1].astype(float)),
				   df.tail(1)[0].values[0].split(","), "d_surface")

for file in ["result", "result_trs"]:
	lab1_values = pd.read_csv("{}.csv".format(file)).groupby(
		['NumThreads', 'Dim'], as_index=False).mean()
